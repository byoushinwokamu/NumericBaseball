...
#define DISPTURN \
  if (dispTurn) displayTurn(game, board[j][0], board[j][1], board[j][2])
...
int NBBot::playGame(NBGame &game, bool dispTurn, bool dispRes)
{
  ...
  {
    if (noans[j]) continue;
    game.playTurn(board[j][0], board[j][1], board[j][2]);
    DISPTURN;
    if (game.getStrike() == 3)
    {
      DISPRES;
    ...
    for (int i = 0; i < 504; i++)
    {
      if (noans[i]) continue;
      int st = 0, ba = 0;
      if (board[i][0] == board[j][0]) st++;
      else if (board[i][0] == board[j][1] || board[i][0] == board[j][2]) ba++;
      if (board[i][1] == board[j][1]) st++;
      else if (board[i][1] == board[j][0] || board[i][1] == board[j][2]) ba++;
      if (board[i][2] == board[j][2]) st++;
      else if (board[i][2] == board[j][0] || board[i][2] == board[j][1]) ba++;
